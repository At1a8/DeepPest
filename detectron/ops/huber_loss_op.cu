...
namespace {
template <typename T>
__global__ void HuberKernel(
    const int n, const T* in, T* out, T tau) {
  // f(x) = 0.5 * x^2 / tau      if |x| < tau
  //        |x| - 0.5 * tau      otherwise
  CUDA_1D_KERNEL_LOOP(index, n) {
    ...
    if (abs_val < tau) {
      ...
      out[index] = tau * (abs_val) - 0.5 * tau * tau;
    }
  }
}

template <typename T>
__global__ void HuberGradientKernel(
    const int n,
    const T* in,
    T* out,
    const T* d_loss_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, n) {
    ...
    if (abs_val < tau) {
      out[index] = norm * d_loss * val;
    } else {
      out[index] = norm * d_loss * tau * ((T(0) < val) - (val < T(0)));
    }
  }
}
} // namespace
...
template<>
bool HuberLossOp<float, CUDAContext>::RunOnDevice() {
  auto& Y_hat     = Input(0);
  auto& Y         = Input(1);
  ...
  // loss weight)
  // l := alpha_out * Huber(alpha_in * (y_hat - y))
  math::Mul<float, CUDAContext>(
...
template<>
bool HuberLossGradientOp<float, CUDAContext>::RunOnDevice() {
  auto& Y_hat      = Input(0);
  auto& Y          = Input(1);
  auto& alpha_in   = Input(2);
  auto& alpha_out  = Input(3);
  ...
  math::Mul<float, CUDAContext>(
      buff_.size(), buff_.data<float>(), alpha_in.data<float>(),
      buff_.mutable_data<float>(), &context_);
  // d_Y_hat := d_avg_loss / N * Huber'(alpha_in * (y_hat - y))
  HuberGradientKernel<float>
  <<<CAFFE_GET_BLOCKS(buff_.size()),
     CAFFE_CUDA_NUM_THREADS,
     0,
     context_.cuda_stream()>>>(
...
REGISTER_CUDA_OPERATOR(HuberLoss,
                       HuberLossOp<float, CUDAContext>);
REGISTER_CUDA_OPERATOR(HuberLossGradient,
                       HuberLossGradientOp<float, CUDAContext>);
}  // namespace caffe2
