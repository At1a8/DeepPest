...
def im_detect_all(model, im, box_proposals, timers=None):
    ...
    if cfg.MODEL.CLASSIFICATION:
        cls_scores = im_classify(model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, timers)
...
def im_classify(model, im, target_scale, target_max_size, timers):

    inputs, im_scales = _get_blobs(im, None, target_scale, target_max_size,)
