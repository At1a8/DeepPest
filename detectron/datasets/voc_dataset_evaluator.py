...
def _get_voc_results_file_template(json_dataset, salt):
    ...
    return os.path.join(root_path, 'results', filename)
...
def voc_info(json_dataset):
    ...
    image_set = json_dataset.name.split('_')[-1]
    #image_set = json_dataset.name[9:]
    #devkit_path = DATASETS[json_dataset.name][DEVKIT_DIR]
    root_path = DATASETS[json_dataset.name][ROOT_DIR]
    assert os.path.exists(root_path), \
        'Root directory {} not found'.format(devkit_path)
    if cfg.MODEL.MASK_ON is True:
        anno_path = os.path.join(
            root_path, 'Annotations', '{:s}.json')
    else:
        anno_path = os.path.join(
            root_path, 'Annotations', '{:s}.xml')
    image_set_path = os.path.join(
        root_path, 'ImageSets', 'Main', image_set + '.txt')
    return dict(
        #year=year,
        image_set=image_set,
        root_path=root_path,
