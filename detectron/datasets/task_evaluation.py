...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False):
    ...
    if _use_json_dataset_evaluator(dataset):
        logger.info(_use_json_dataset_evaluator(dataset))
    ...
    if _use_cityscapes_evaluator(dataset):
    ...
    if not True in [_use_json_dataset_evaluator(dataset), _use_cityscapes_evaluator(dataset), _use_voc_evaluator(dataset)]:
...
def _use_json_dataset_evaluator(dataset):
    ...
    return cfg.TEST.COCO_EVALUATOR
...
def _use_cityscapes_evaluator(dataset):
    ...
    return cfg.TEST.CITYSCAPES_EVALUATOR
