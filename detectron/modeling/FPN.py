...
import detectron.modeling.Perceptual_ResNet as Perceptual_ResNet
...
import detectron.modeling.VGG as VGG
import detectron.modeling.VGG_CNN_M_1024 as VGG_CNN_M_1024
import detectron.modeling.ZF as ZF
...
def add_fpn_Perceptual_ResNet50_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, Perceptual_ResNet.add_ResNet50_conv5_body, fpn_level_info_Perceptual_ResNet50_conv5
    )
...
def add_fpn_VGG16_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, VGG.add_VGG16_conv5_body, fpn_level_info_VGG16_conv5
    )

def add_fpn_VGG19_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, VGG.add_VGG19_conv5_body, fpn_level_info_VGG19_conv5
    )
...
def add_fpn_ZF_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, ZF.add_ZF_conv5_body, fpn_level_info_ZF_conv5
    )
...
def add_fpn_Inception_v4_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, Inception.add_Inception_v4_conv5_body, fpn_level_info_Inception_v4_conv5
    )

def add_fpn_Inception_ResNet_v2_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, Inception.add_Inception_ResNet_v2_conv5_body, fpn_level_info_Inception_ResNet_v2_conv5
    )
...
def add_fpn_DenseNet121_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, DenseNet.add_DenseNet121_conv5_body, fpn_level_info_DenseNet121_conv5
    )

def add_fpn_DenseNet161_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, DenseNet.add_DenseNet161_conv5_body, fpn_level_info_DenseNet161_conv5
    )

def add_fpn_DenseNet169_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, DenseNet.add_DenseNet169_conv5_body, fpn_level_info_DenseNet169_conv5
    )

def add_fpn_DenseNet201_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, DenseNet.add_DenseNet201_conv5_body, fpn_level_info_DenseNet201_conv5
    )
...
def add_fpn_MobileNet_conv5_body(model):
    return add_fpn_onto_conv_body(
        model, MobileNet.add_MobileNet_conv5_body, fpn_level_info_MobileNet_conv5
    )
...
def add_topdown_lateral_module(
    ...
    lat_narrow = model.net.SpatialNarrowAs([lat, td],lat+'_narrow')
    model.net.Sum([lat_narrow, td], fpn_bottom)
...
def fpn_level_info_Perceptual_ResNet50_conv5():
    return FpnLevelInfo(
        blobs=('conv6', 'res5_2_sum', 'res4_5_sum', 'res3_3_sum'),
        dims=(2560, 2048, 1024, 512),
        spatial_scales=(1. / 64., 1. / 32., 1. / 16., 1. / 8.)
    )
...
def fpn_level_info_VGG19_conv5():
    return FpnLevelInfo(
        blobs=('conv5_4', 'conv4_3', 'conv3_3', 'conv2_2'),
        dims=(512, 512, 256, 128),
        spatial_scales=(1. / 16., 1. / 8., 1. / 4., 1. / 2.)
    )
...
def fpn_level_info_VGG_CNN_M_1024_conv5():
    return FpnLevelInfo(
        blobs=('conv5', 'conv4', 'conv3', 'conv2'),
        dims=(512, 512, 512, 256),
        spatial_scales=(1. / 8., 1. / 8., 1. / 8., 1. / 4.)
    )

def fpn_level_info_ZF_conv5():
    return FpnLevelInfo(
        blobs=('conv5', 'conv4', 'conv3', 'conv2'),
        dims=(256, 384, 384, 256),
        spatial_scales=(1. / 16., 1. / 16., 1. / 16., 1. / 8.)
    )
...
def fpn_level_info_Inception_v4_conv5():
    return FpnLevelInfo(
        blobs=('inception_c3_concat', 'inception_b7_concat', 'inception_a4_concat', 'inception_stem3'),
        dims=(1536, 1024, 384, 384),
        spatial_scales=(1. / 32, 1. / 16, 1. / 8., 1. / 8.)
    )

def fpn_level_info_Inception_ResNet_v2_conv5():
    return FpnLevelInfo(
        blobs=('conv6_1x1', 'inception_resnet_v2_b20_residual_eltwise', 'inception_resnet_v2_a10_residual_eltwise', 'conv4_3x3'),
        dims=(1536, 1088, 320, 192),
        spatial_scales=(1. / 32, 1. / 16, 1. / 8., 1. / 4.)
    )

def fpn_level_info_DenseNet121_conv5():
    return FpnLevelInfo(
        blobs=('conv5_blk_bn', 'conv4_blk', 'conv3_blk', 'conv2_blk'),
        dims=(1024, 512, 256, 128),
        spatial_scales=(1. / 32., 1. / 16., 1. / 8., 1. / 4.)
    )

def fpn_level_info_DenseNet161_conv5():
    return FpnLevelInfo(
        blobs=('conv5_blk_bn', 'conv4_blk', 'conv3_blk', 'conv2_blk'),
        dims=(2208, 1056, 384, 192),
        spatial_scales=(1. / 28., 1. / 14., 1. / 7., 1. / 4.)
    )

def fpn_level_info_DenseNet169_conv5():
    return FpnLevelInfo(
        blobs=('relu5_blk', 'conv4_blk', 'conv3_blk', 'conv2_blk'),
        dims=(1664, 640, 256, 128),
        spatial_scales=(1. / 28., 1. / 14., 1. / 7., 1. / 4.)
    )

def fpn_level_info_DenseNet201_conv5():
    return FpnLevelInfo(
        blobs=('relu5_blk', 'conv4_blk', 'conv3_blk', 'conv2_blk'),
        dims=(1920, 896, 256, 128),
        spatial_scales=(1. / 28., 1. / 14., 1. / 7., 1. / 4.)
    )

def fpn_level_info_MobileNet_conv5():
    return FpnLevelInfo(
        blobs=('conv6_sep', 'conv4_2_sep', 'conv3_2_sep', 'conv2_2_sep'),
        dims=(1024, 512, 256, 128),
        spatial_scales=(1. / 32., 1. / 16., 1. / 8., 1. / 4.)
    )
