...
def add_ResNet_convX_body(model, block_counts, freeze_at=0):
    ...
    model.Conv('pool1', 'conv2', 64, 128, 3, pad=0, stride=2)
    model.AffineChannel('conv2', 'conv2', dim=128)
    model.Relu('conv2', 'conv2')
    model.Conv('conv2', 'conv3', 128, 256, 3, pad=0, stride=2)
    model.AffineChannel('conv3', 'conv3', dim=256)
    model.Relu('conv3', 'conv3')
    model.Conv('conv3', 'conv4', 256, 512, 3, pad=1, stride=2)
    model.AffineChannel('conv4', 'conv4', dim=512)
    ...
    if len(block_counts) == 4:
        ...
        if freeze_at == 5:
            model.StopGradient(s, s)
        model.Concat(['conv4','res5_2_sum'], 'conv5')
        model.Conv('conv5', 'conv6', 2560, 2560, 3, pad=0, stride=2)
        model.AffineChannel('conv6', 'conv6', dim=2560)
        s = model.Relu('conv6', 'conv6')
        dim_in = 2560
        return s, dim_in, 1. / 64. * cfg.RESNETS.RES5_DILATION
    else:
