...
def add_ZF_conv5_body(model):
    model.Conv('data', 'conv1', 3, 96, 7, pad=0, stride=2)
    model.Relu('conv1', 'conv1')
    model.LRN('conv1', 'norm1', size=3, alpha=0.0005, beta=0.75, bias=2.)
    ...
    model.LRN('conv2', 'norm2', size=3, alpha=0.0005, beta=0.75, bias=2.)
    model.MaxPool('norm2', 'pool2', kernel=3, pad=1, stride=2)
    model.Conv('pool2', 'conv3', 256, 384, 3, pad=1, stride=1)
    model.Relu('conv3', 'conv3')
    model.Conv('conv3', 'conv4', 384, 384, 3, pad=1, stride=1)
    model.Relu('conv4', 'conv4')
    model.Conv('conv4', 'conv5', 384, 256, 3, pad=1, stride=1)
    blob_out = model.Relu('conv5', 'conv5')
    return blob_out, 256, 1. / 16.


def add_ZF_roi_fc_head(model, blob_in, dim_in, spatial_scale):
